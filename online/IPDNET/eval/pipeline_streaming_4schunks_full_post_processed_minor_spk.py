...
Loads IPDNET and DSENet once, then chops each long Respeaker mic-array
recording into 4 s chunks and streams the enhanced source-2/minor speaker
into a SimulStreaming Whisper server. For each chunk:

1. Run SSL once on the representative multichannel mixture.
2. Convert SSL output to up to three DOAs.
3. Run DSENet once with one DOA input per source declared in the filename.
4. Select the enhanced output whose predicted DoA is closest to filename DoA2
   and send it to streaming Whisper.

Filename DoA2 is used as the source-2 target direction. Reference transcripts
are loaded from ``Respeaker_real/text/src2`` with ``200.txt`` mapping to
``fileid_0`` and ``299.txt`` mapping to ``fileid_99``.
"""
...
import torch
from scipy.signal import butter, filtfilt, istft, resample_poly, stft
...
def target_source_doa(gt_source_doas: str, source_index: int) -> Optional[int]:
    doas = parse_gt_source_doas(gt_source_doas)
    if source_index < 1 or source_index > len(doas):
        return None
    return doas[source_index - 1]


def select_enhanced_closest_to_doa(
    enhanced_batch: Sequence[np.ndarray],
    pred_doas: Sequence[int],
    target_doa: int,
) -> Tuple[int, float]:
    if not enhanced_batch:
        raise ValueError("Cannot select enhanced signal from an empty batch.")
    if len(enhanced_batch) != len(pred_doas):
        raise ValueError(
            f"Enhanced output count ({len(enhanced_batch)}) does not match predicted DoA count ({len(pred_doas)})."
        )
    errors = [circular_angle_error_deg(pred_doa, target_doa) for pred_doa in pred_doas]
    selected_idx = int(np.argmin(errors))
    return selected_idx, float(errors[selected_idx])


def order_candidates_by_gt_doa(
    pred_doas: Sequence[int],
    gt_source_doas: str,
) -> List[Tuple[int, int, Optional[int], Optional[int], Optional[float]]]:
    valid_gt_doas = [
        (source_idx, doa)
        for source_idx, doa in enumerate(parse_gt_source_doas(gt_source_doas), start=1)
        if doa is not None
    ]
    if not valid_gt_doas:
        return [
            (order_idx, candidate_idx, None, None, None)
            for order_idx, candidate_idx in enumerate(range(len(pred_doas)), start=1)
        ]

    candidate_indices = list(range(len(pred_doas)))
    pair_errors = {
        (candidate_idx, source_idx): circular_angle_error_deg(pred_doas[candidate_idx], gt_doa)
        for candidate_idx in candidate_indices
        for source_idx, gt_doa in valid_gt_doas
    }

    # There are normally only three sources, so exhaustive assignment keeps this dependency-free.
    from itertools import combinations, permutations

    assignment: Dict[int, Tuple[int, int, float]] = {}
    if len(candidate_indices) <= len(valid_gt_doas):
        best_cost = float("inf")
        best_pairs: List[Tuple[int, int, int, float]] = []
        for gt_subset in combinations(valid_gt_doas, len(candidate_indices)):
            for perm in permutations(gt_subset):
                pairs = [
                    (candidate_idx, source_idx, gt_doa, pair_errors[(candidate_idx, source_idx)])
                    for candidate_idx, (source_idx, gt_doa) in zip(candidate_indices, perm)
                ]
                cost = sum(pair[3] for pair in pairs)
                if cost < best_cost:
                    best_cost = cost
                    best_pairs = pairs
        assignment = {
            candidate_idx: (source_idx, gt_doa, error)
            for candidate_idx, source_idx, gt_doa, error in best_pairs
        }
    else:
        best_cost = float("inf")
        best_pairs = []
        for candidate_subset in combinations(candidate_indices, len(valid_gt_doas)):
            for perm in permutations(candidate_subset):
                pairs = [
                    (candidate_idx, source_idx, gt_doa, pair_errors[(candidate_idx, source_idx)])
                    for candidate_idx, (source_idx, gt_doa) in zip(perm, valid_gt_doas)
                ]
                cost = sum(pair[3] for pair in pairs)
                if cost < best_cost:
                    best_cost = cost
        ...
        for candidate_idx in candidate_indices:
            if candidate_idx not in assignment:
                source_idx, error = closest_gt_source(pred_doas[candidate_idx], gt_source_doas)
                gt_doa = parse_gt_source_doas(gt_source_doas)[source_idx - 1] if source_idx is not None else None
                assignment[candidate_idx] = (source_idx, gt_doa, error)

    ordered = []
    for candidate_idx in candidate_indices:
        source_idx, gt_doa, error = assignment[candidate_idx]
        source_order = source_idx if source_idx is not None else 999
        ordered.append((source_order, candidate_idx, source_idx, gt_doa, error))
    ordered.sort(key=lambda item: (item[0], item[4] if item[4] is not None else float("inf"), item[1]))
    return [
        (order_idx, candidate_idx, source_idx, gt_doa, error)
        for order_idx, (_, candidate_idx, source_idx, gt_doa, error) in enumerate(ordered, start=1)
    ]
...
class EvalChunk:
    ...
    mic_file: str
    source_count: int
    gt_target_spk2_doa: Optional[int]
    gt_source_doas: str
        ...
        for start_sample in range(0, info.frames, chunk_samples):
            ...
            chunks.append(
                EvalChunk(
                    ...
                    gt_target_spk2_doa=doas[1] if len(doas) > 1 else None,
...
def highpass_audio(audio: np.ndarray, sr: int, cutoff_hz: float) -> np.ndarray:
    if cutoff_hz <= 0:
...
def soft_knee_compressor_audio(
    audio: np.ndarray,
    threshold_db: float,
    ratio: float,
    makeup_db: float,
    ...
    if ratio <= 1.0:
        return (audio * db_to_amp(makeup_db)).astype(np.float32)
...
def peak_limit_audio(audio: np.ndarray, limit: float) -> np.ndarray:
    limit = float(np.clip(limit, 0.01, 1.0))
    return np.clip(audio, -limit, limit).astype(np.float32)


def normalize_rms_audio(audio: np.ndarray, target_rms_dbfs: float, peak_limit_value: float) -> np.ndarray:
...
def match_rms_audio(
    ...
    return peak_limit_audio(audio * gain, peak_limit_value)


def postprocess_enhanced_audio(audio: np.ndarray, sr: int, args: argparse.Namespace) -> np.ndarray:
    audio = np.asarray(audio, dtype=np.float32)
    if audio.ndim > 1:
        audio = audio.mean(axis=1)
    original_audio = audio.copy()
    audio = highpass_audio(audio, sr, args.post_highpass_hz)
    audio = spectral_denoise_audio(
        audio,
        sr,
        noise_reduce_db=args.post_noise_reduce_db,
        noise_percentile=args.post_noise_percentile,
        noise_profile_percentile=args.post_noise_profile_percentile,
        n_fft=args.post_n_fft,
        hop_length=args.post_hop_length,
    )
    audio = soft_knee_compressor_audio(
        audio,
        threshold_db=args.post_compressor_threshold_db,
        ratio=args.post_compressor_ratio,
        makeup_db=args.post_makeup_db,
    )
    audio = peak_limit_audio(audio, args.post_peak_limit)
    if args.post_preserve_chunk_rms:
        audio = match_rms_audio(
            audio,
            original_audio,
            args.post_peak_limit,
            args.post_preserve_chunk_rms_blend,
        )
    elif args.post_target_rms_dbfs is not None:
        audio = normalize_rms_audio(audio, args.post_target_rms_dbfs, args.post_peak_limit)
    return peak_limit_audio(audio, args.post_peak_limit)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--save_enhanced", action="store_true", help="Save the selected source-2 enhanced wav.")
    parser.add_argument(
        "--save_all_enhanced",
        action="store_true",
        help="Save every enhanced DSENet output for each chunk, not only the selected source-2 output.",
    )
    parser.add_argument("--all_enhanced_subdir", type=str, default="pipeline_realtime_enhanced_all")
    parser.add_argument(
    ...
    parser.add_argument(
        "--minor_text_dir",
        type=Path,
        default=SCRIPT_DIR / "Respeaker_real" / "text" / "src2",
        help="Reference transcript folder for source 2/minor speaker.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=float,
        default=30.0,
        help="Selected DoA is target-correct when it is within this many degrees of filename DoA2.",
    )
    parser.add_argument("--input_gain", type=float, default=1.0)
    parser.add_argument(
        "--postprocess_enhanced",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Post-process the selected source-2 enhanced chunk before saving, concatenating, and ASR.",
    )
    parser.add_argument("--post_noise_reduce_db", type=float, default=500.0)
    parser.add_argument("--post_noise_percentile", type=float, default=60.0)
    parser.add_argument("--post_noise_profile_percentile", type=float, default=95.0)
    parser.add_argument("--post_highpass_hz", type=float, default=100.0)
    parser.add_argument("--post_compressor_threshold_db", type=float, default=-28.0)
    parser.add_argument("--post_compressor_ratio", type=float, default=1.0)
    parser.add_argument("--post_makeup_db", type=float, default=0.0)
    parser.add_argument(
        "--post_preserve_chunk_rms",
        action=argparse.BooleanOptionalAction,
        default=True,
        help="After post-processing each selected 4 s chunk, restore it toward its original enhanced RMS.",
    )
    parser.add_argument("--post_preserve_chunk_rms_blend", type=float, default=1.0)
    parser.add_argument(
        "--post_target_rms_dbfs",
        type=float,
        default=None,
        help="Per-chunk RMS target used only when --no-post_preserve_chunk_rms is set.",
    )
    parser.add_argument("--post_peak_limit", type=float, default=0.95)
    parser.add_argument("--post_n_fft", type=int, default=512)
    parser.add_argument("--post_hop_length", type=int, default=128)
...
def main() -> None:
    ...
    if not args.minor_text_dir.is_dir():
        raise FileNotFoundError(f"Minor speaker text folder not found: {args.minor_text_dir}")
    ...
    all_enhanced_dir = args.out_dir / args.all_enhanced_subdir
    if args.save_all_enhanced:
        all_enhanced_dir.mkdir(parents=True, exist_ok=True)
    ...
    target_spk2_doas: Dict[int, int] = {}
    for path in target_files:
        fileid, _, doas = parse_respeaker_recording_name(path)
        if len(doas) > 1 and doas[1] is not None:
            target_spk2_doas[fileid] = doas[1]
    target_spk2_texts = load_offset_respeaker_texts(args.minor_text_dir, args.minor_text_fileid_offset)
    ...
    print(f"Minor speaker text folder: {args.minor_text_dir}")
    print(f"Minor speaker text fileid offset: {args.minor_text_fileid_offset}")
    print(f"Respeaker chunk size: {args.chunk_sec:.3f}s")
    print(f"Include partial final chunk: {args.include_partial_chunk}")
    print(f"Post-process selected enhanced audio: {args.postprocess_enhanced}")
    print(f"Save all enhanced candidates: {args.save_all_enhanced}")
    ...
    print(f"Target spk2 DOA references: {len(target_spk2_doas)}")
    print(f"Target spk2 text references: {len(target_spk2_texts)}")
    ...
    try:
        for eval_chunk in tqdm(
            ...
            wav_tc = np.clip(wav_tc * args.input_gain, -1.0, 1.0)     # add argument for raw input gain
            ...
            gt_target_doa = eval_chunk.gt_target_spk2_doa
            if gt_target_doa is None:
                missing_target_gt += 1
                print(f"fileid={fileid} chunk={chunk_index}: filename DoA2 is missing, skipped.")
                continue

            selected_idx, selected_doa_error = select_enhanced_closest_to_doa(
                enhanced_batch,
                pred_doas,
                gt_target_doa,
            )
            selected_doa = pred_doas[selected_idx]
            selected_rms = signal_rms(enhanced_batch[selected_idx])
            selected_save_name = (
                f"enhanced_fileid_{fileid}_chunk{chunk_index}_"
                f"targetspk2_gt{gt_target_doa}_err{selected_doa_error:.1f}_"
                f"pred{selected_doa}_idx{selected_idx}.wav"
            )
            enhanced_for_asr = enhanced_batch[selected_idx][:valid_samples]
            postprocess_sec = 0.0
            if args.postprocess_enhanced:
                post_start = time.perf_counter()
                enhanced_for_asr = postprocess_enhanced_audio(enhanced_for_asr, sr, args)
                postprocess_sec = time.perf_counter() - post_start
            postprocessed_rms = signal_rms(enhanced_for_asr)
            target_doa_correct = (
                int(selected_doa_error <= args.target_doa_correct_threshold)
                if selected_doa_error is not None
                else None
            )
            closest_source_index, closest_gt_error = closest_gt_source(selected_doa, eval_chunk.gt_source_doas)
            selected_closer_to_nontarget = int(
                closest_source_index is not None
                and closest_source_index != 2
            ...
            if args.save_all_enhanced:
                candidate_order = order_candidates_by_gt_doa(pred_doas, eval_chunk.gt_source_doas)
                for order_idx, candidate_idx, matched_source_idx, matched_gt_doa, matched_error in candidate_order:
                    candidate_audio = enhanced_batch[candidate_idx]
                    candidate_doa = pred_doas[candidate_idx]
                    candidate_for_save = candidate_audio[:valid_samples]
                    if args.postprocess_enhanced:
                        candidate_for_save = postprocess_enhanced_audio(candidate_for_save, sr, args)
                    selected_tag = "_selected" if candidate_idx == selected_idx else ""
                    gt_source_tag = f"gtspk{matched_source_idx}" if matched_source_idx is not None else "gtspkNA"
                    gt_doa_tag = f"gt{matched_gt_doa}" if matched_gt_doa is not None else "gtNA"
                    err_tag = f"err{matched_error:.1f}" if matched_error is not None else "errNA"
                    candidate_save_name = (
                        f"enhanced_fileid_{fileid}_chunk{chunk_index}_"
                        f"ord{order_idx}_{gt_source_tag}_{gt_doa_tag}_{err_tag}_"
                        f"pred{candidate_doa}_idx{candidate_idx}{selected_tag}.wav"
                    )
                    sf.write(str(all_enhanced_dir / candidate_save_name), candidate_for_save, sr)
            ...
            frontend_compute_sec = ipd_sec + dse_sec + postprocess_sec
            ...
            scene_results.append(
                SceneTiming(
                    ...
                    postprocessed_rms=postprocessed_rms,
                    selected_enhanced_file=selected_save_name,
                    gt_target_spk2_doa=gt_target_doa,
    ...
    for fileid, rows in sorted(rows_by_fileid.items()):
        ...
        text_path = target_spk2_texts.get(fileid)
        ...
        if args.save_concatenated_enhanced:
            ...
            if enhanced_chunks:
                ...
                concatenated_enhanced_file = f"enhanced_fileid_{fileid}_concatenated_targetspk2.wav"
                sf.write(
        ...
        scene_wer_results.append(
            SceneWer(
                ...
                gt_target_spk2_doa=target_spk2_doas.get(fileid),
    ...
    details_csv = args.out_dir / f"pipeline_streaming_{args.whisper_model}_details_1asr_4s_full_post_processed.csv"
    ...
    scene_wer_csv = args.out_dir / f"pipeline_streaming_{args.whisper_model}_scene_wer_1asr_4s_full_post_processed.csv"
    ...
    summary = {
        ...
        "realtime_tolerance_sec": args.realtime_tolerance_sec,
        "device": args.device,
        "dse_batch_size": "source_count_from_filename",
        "selection": "closest_predicted_doa_to_filename_doa2",
        ...
        "save_all_enhanced": args.save_all_enhanced,
        "all_enhanced_dir": str(all_enhanced_dir) if args.save_all_enhanced else "",
        ...
        "postprocess_enhanced": args.postprocess_enhanced,
        "post_noise_reduce_db": args.post_noise_reduce_db,
        "post_noise_percentile": args.post_noise_percentile,
        "post_noise_profile_percentile": args.post_noise_profile_percentile,
        "post_highpass_hz": args.post_highpass_hz,
        "post_compressor_threshold_db": args.post_compressor_threshold_db,
        "post_compressor_ratio": args.post_compressor_ratio,
        "post_makeup_db": args.post_makeup_db,
        "post_preserve_chunk_rms": args.post_preserve_chunk_rms,
        "post_preserve_chunk_rms_blend": args.post_preserve_chunk_rms_blend,
        "post_target_rms_dbfs": args.post_target_rms_dbfs,
        "post_peak_limit": args.post_peak_limit,
        "post_n_fft": args.post_n_fft,
        "post_hop_length": args.post_hop_length,
        ...
        "target_spk2_doa_references": len(target_spk2_doas),
        "target_spk2_text_references": len(target_spk2_texts),
        ...
        "missing_target_spk2_gt": missing_target_gt,
        ...
        "mean_postprocess_sec": float(np.mean([r.postprocess_sec for r in scene_results])) if scene_results else 0.0,
    ...
    summary_json = args.out_dir / f"pipeline_streaming_{args.whisper_model}_summary_1asr_4s_full_post_processed.json"
    summary_json.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    summary_csv = args.out_dir / f"pipeline_streaming_{args.whisper_model}_summary_1asr_4s_full_post_processed.csv"
    ...
    print(f"Wrong-source selection rate: {summary['wrong_speaker_selection_rate']:.4f}")
    print(
    ...
    print(f"Missing target spk2 DOA references: {summary['missing_target_spk2_gt']}")
    print(
        "Selected source-2 DOA error vs DoA2 GT: "
    ...
    print(
        ...
        f"postprocess={summary['mean_postprocess_sec']:.3f}s, "
    ...
    if args.save_all_enhanced:
        print(f"Saved all enhanced candidates: {all_enhanced_dir}")
    if args.save_concatenated_enhanced:
        print(f"Saved concatenated enhanced audio folder: {concatenated_enhanced_dir}")
