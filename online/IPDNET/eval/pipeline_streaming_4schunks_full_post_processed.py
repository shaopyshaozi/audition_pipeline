...
recording into 4 s chunks and streams the selected enhanced dominant speaker
...
The dominant speaker is selected as the loudest enhanced DSENet output.
Filename DoA1 is used only as comparison metadata, and ``0.txt`` to ``99.txt``
from Respeaker_real/text/src1 are used as the dominant-speaker reference
transcripts.
"""
...
def select_loudest_enhanced(enhanced_batch: Sequence[np.ndarray]) -> Tuple[int, float]:
    ...
    scores = [np.percentile(np.abs(enhanced), 99) for enhanced in enhanced_batch]
    selected_idx = int(np.argmax(scores))
    return selected_idx, scores[selected_idx]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--save_all_enhanced",
        action="store_true",
        help="Save every enhanced DSENet output for each chunk, not only the selected dominant output.",
    )
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Post-process the selected loudest enhanced chunk before saving, concatenating, and ASR.",
    )
    ...
    for fileid, rows in sorted(rows_by_fileid.items()):
        ...
        if args.save_concatenated_enhanced:
            ...
            if enhanced_chunks:
                ...
                concatenated_enhanced_file = f"enhanced_fileid_{fileid}_concatenated_loudest.wav"
                sf.write(
