...
Loads DSENet once, then streams each saved multichannel 4 s wav
one scene at a time into a SimulStreaming Whisper server. For each scene:

1. Read the dominant speaker GT DoA from the clean spk1 filename.
2. Run DSENet once with that GT DoA input.
3. Send the enhanced output to streaming Whisper.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run GT-DoA -> DSENet -> streaming Whisper realtime benchmark.")
...
def main() -> None:
    ...
    print("DSENet batch size: 1 GT dominant-spk1 DoA per chunk")
    ...
    print("Using ground-truth dominant spk1 DoA from clean filenames; IPDNET is not run.")
    ...
    try:
        for (fileid, chunk_index), target_paths in tqdm(
            ...
            if gt_dominant_doa is None:
                ...
                print(f"fileid={fileid} chunk={chunk_index}: dominant spk1 GT DoA is missing, skipped.")
                continue
            pred_doas = [gt_dominant_doa]
            ipd_sec = 0.0
            ...
            selected_save_name = (
                ...
                f"gt{gt_dominant_doa}_pred{selected_doa}_idx{selected_idx}.wav"
            )
    ...
    summary = {
        ...
        "device": args.device,
        "dse_batch_size": 1,
        "doa_source": "gt_dominant_spk1_doa_from_clean_filename",
        "selection": "gt_dominant_spk1_doa",
    ...
    print(
        "Selected GT DOA error vs spk1 GT: "
