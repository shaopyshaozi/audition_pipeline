...
def infer_speaker_tag(data_dir: str) -> str:
    dataset_name = os.path.basename(os.path.dirname(data_dir))
    matches = re.findall(r"\d+spk", dataset_name)
    if matches:
        return matches[-1]
    return dataset_name.split("_")[-1]


def parse_clean_wav_name(clean_wav_name: str) -> Dict[str, Any]:
    stem, ext = os.path.splitext(clean_wav_name)
    if ext.lower() != ".wav":
        raise ValueError(f"Expected a wav file, got {clean_wav_name}")

    parts = stem.split("_")
    if len(parts) < 5 or parts[0] != "clean" or parts[1] != "fileid":
        raise ValueError(
            "Expected clean filename like "
            "clean_fileid_<id>[_chunk<n>]_doa<angle>[_width<w>]_spk<k>.wav, "
            f"got {clean_wav_name}"
        )

    doa_idx = next((i for i, p in enumerate(parts) if re.fullmatch(r"doa\d+", p)), None)
    if doa_idx is None:
        raise ValueError(f"Could not find DOA token in {clean_wav_name}")

    width_idx = next((i for i, p in enumerate(parts) if re.fullmatch(r"width\d+", p)), None)
    file_id_parts = parts[1:doa_idx]
    doa_token = parts[doa_idx]
    width_token = parts[width_idx] if width_idx is not None else None

    return {
        "file_id": "_".join(file_id_parts),
        "mic_prefix": "_".join(["mic", *file_id_parts, doa_token] + ([width_token] if width_token else [])),
        "doa": int(doa_token[len("doa"):]),
        "width": int(width_token[len("width"):]) if width_token else 30,
    }
...
class MultimicDataset(Dataset):
    def __init__(self, data_dir: str, sample_rate: int = 16000, cut_len: int = 4):
        ...
        self.noisy_wav_names = set(os.listdir(self.noisy_dir))
        self.dataset_tag = infer_speaker_tag(data_dir)
    ...
    def __getitem__(self, idx):
        ...
        clean_info = parse_clean_wav_name(self.clean_wav_name[idx])
        expected_noisy_name = f"{clean_info['mic_prefix']}_{self.dataset_tag}.wav"
        if expected_noisy_name in self.noisy_wav_names:
            noisy_name = expected_noisy_name
        else:
            noisy_matches = [
                name for name in self.noisy_wav_names
                if name.startswith(f"{clean_info['mic_prefix']}_") and name.endswith(".wav")
            ]
            if len(noisy_matches) != 1:
                raise FileNotFoundError(
                    f"Could not resolve a unique mic file for {self.clean_wav_name[idx]}. "
                    f"Expected {expected_noisy_name}; found {noisy_matches}"
                )
            noisy_name = noisy_matches[0]
        noisy_file = os.path.join(self.noisy_dir, noisy_name)
        ...
        target_name = f"enhance_{clean_info['file_id']}_doa_width.wav"
