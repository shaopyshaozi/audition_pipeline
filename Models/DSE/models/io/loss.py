...
def RIMag(preds, target, stft_func,  **kwargs):
    ...
    preds_stft  = stft_func(preds)[0]   
    target_stft  = stft_func(target)[0] 
...
class MultiResolutionSTFTLoss(torch.nn.Module):
    ...
    def forward(self, preds, target, **kwargs):
        ...
        wav_loss = torch.linalg.norm( preds - target, dim = -1, ord = 1 )
        # return sc_loss + mag_loss + 0.5*negsidr
        return mag_loss + 0.6*negsidr
...
class Loss(nn.Module):
    ...
    def __init__(self, loss_func: Callable, pit: bool = False, loss_func_kwargs: Dict[str, Any] = dict(), ):
        super().__init__()
