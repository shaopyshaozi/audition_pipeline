...
class PredDOA(nn.Module):
	def __init__(self,
		  ...
		  max_num_sources = 1,
		  max_track = 3,   #  This is Number of tracks = 3 speakers--------------------------------------------------------------
...
class PredDOA_Inference(nn.Module):
    def __init__(self,
                 source_num_mode='UnkNum',
                 max_num_sources=1,
                 max_track=3,
                 ...
                 dev='cuda',
                 mic_location=None):
        super(PredDOA_Inference, self).__init__()
        ...
        self.gerdpipd = DPIPD2(
            ndoa_candidate=[res_the, res_phi],
            mic_location=mic_location,
            nf=int(self.nfft / 2) + 1,
            fre_max=self.fre_max,
            ch_mode=self.ch_mode,
    ...
    def forward(self, pred_batch, idx=None, dir_name=None):
        pred_batch = self.pred2DOA(pred_batch)

        doa_est = pred_batch[0] * 180 / np.pi
        vad_est = pred_batch[1].to(self.dev)

        if idx is not None and dir_name is not None:
            os.makedirs(dir_name, exist_ok=True)

            np.save(dir_name + str(idx) + '_doaest', doa_est.cpu().numpy())
            np.save(dir_name + str(idx) + '_vadest', vad_est.cpu().numpy())
            np.save(dir_name + str(idx) + '_mic', self.mic_location)

        return {
            "doa_est": doa_est,
            "vad_est": vad_est,
            "pred_ipd": pred_batch[2],
        }

    def pred2DOA(self, pred_batch):
        nb, nt, ndoa, nmic, nmax = pred_batch.shape

        pred_ipd = pred_batch.permute(
            0, 3, 1, 2, 4
        ).reshape(nb * nmic, nt, ndoa, nmax)

        return_pred_batch_doa = torch.zeros(
            (nb, nt, 2, self.max_track),
            device=self.dev
        )

        return_pred_batch_vad = torch.zeros(
            (nb, nt, self.max_track),
            device=self.dev
        )

        for i in range(self.max_track):
            pred_batch_temp = self.pred2DOA_track(
                pred_ipd[:, :, :, i]
            )
        ...
        return [
            return_pred_batch_doa,
            return_pred_batch_vad,
            pred_ipd
        ]

    def pred2DOA_track(self, pred_batch=None, time_pool_size=None):
        """
        Convert Estimated IPD of one track to DOA
        """
        ...
        if pred_batch is not None:
            ...
            dpipd_template_sbatch = np.concatenate(
                (
                    dpipd_template_sbatch.real[:, :, self.fre_range_used, :],
                    dpipd_template_sbatch.imag[:, :, self.fre_range_used, :]
                ),
                axis=2
            ).astype(np.float32)

            # (nb, nele, nazi, 2nf, nmic-1)
            dpipd_template = np.tile(
                dpipd_template_sbatch[np.newaxis, :, :, :, :],
                [nb, 1, 1, 1, 1]
            )

            dpipd_template = torch.from_numpy(dpipd_template)

            # rebatch from (nb*nmic, nt, 2nf)
            # to (nb, nt, 2nf, nmic)
            pred_rebatch = self.removebatch(
                pred_batch,
                nb
            ).permute(0, 2, 3, 1)
            ...
            pred_rebatch_flat = pred_rebatch.contiguous().view(
                nb,
                nt,
                -1
            )

            dpipd_template_flat = dpipd_template.contiguous().view(
                nb,
                nele,
                nazi,
                -1
            ...
            pred_rebatch_expanded = pred_rebatch_flat.unsqueeze(2).unsqueeze(2)

            dpipd_template_expanded = dpipd_template_flat.unsqueeze(1)

            mse = torch.mean(
                (
                    pred_rebatch_expanded -
                    dpipd_template_expanded
                ) ** 2,
                dim=-1
            )
            ...
            pred_DOAs = torch.zeros(
                (nb, nt, 2, self.max_num_sources),
                dtype=torch.float32,
                requires_grad=False
            ).to(self.dev)

            pred_VADs = torch.zeros(
                (nb, nt, self.max_num_sources),
                dtype=torch.float32,
                requires_grad=False
            ).to(self.dev)
            ...
            for source_idx in range(self.max_num_sources):
                ...
                map = pred_spatial_spectrum

                max_flat_idx = map.reshape(
                    (nb, nt, -1)
                ).argmin(2)

                ele_max_idx, azi_max_idx = np.unravel_index(
                    max_flat_idx.cpu().numpy(),
                    map.shape[2:]
                )

                ele_candidate = np.linspace(
                    np.pi / 2,
                    np.pi / 2,
                    nele
                )

                azi_candidate = np.linspace(
                    -np.pi,
                    np.pi,
                    nazi
                )
                ...
                pred_DOA = np.stack(
                    (
                        ele_candidate[ele_max_idx],
                        azi_candidate[azi_max_idx]
                    ),
                    axis=-1
                )

                pred_DOA = torch.from_numpy(pred_DOA).to(self.dev)

                pred_DOAs[:, :, :, source_idx] = pred_DOA

                max_dpipd_template = torch.zeros(
                    (nb, nt, nf, nmic),
                    dtype=torch.float32,
                    requires_grad=False
                ).to(self.dev)
                ...
                for b_idx in range(nb):
                    for t_idx in range(nt):
                        ...
                        max_dpipd_template[b_idx, t_idx, :, :] = \
                            dpipd_template[
                                b_idx,
                                ele_max_idx[b_idx, t_idx],
                                azi_max_idx[b_idx, t_idx],
                                :,
                                :
                            ] * 1.0
