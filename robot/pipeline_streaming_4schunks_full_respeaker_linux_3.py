...
def circular_distance_deg_array(angles_deg: np.ndarray, center_deg: float) -> np.ndarray:
    return np.abs((angles_deg - center_deg + 180.0) % 360.0 - 180.0)


def assign_points_to_circular_centers(angles_deg: np.ndarray, centers_deg: Sequence[float]) -> np.ndarray:
    centers_np = np.asarray(centers_deg, dtype=np.float32)
    if centers_np.size == 0:
        return np.full(angles_deg.shape[0], -1, dtype=np.int32)
    distances = np.stack(
        [circular_distance_deg_array(angles_deg, float(center)) for center in centers_np],
        axis=1,
    )
    return np.argmin(distances, axis=1).astype(np.int32)


def separated_peak_cluster_doas(
    ...
    if angles_deg.size < num_sources:
        return [], np.full(angles_deg.shape[0], -1, dtype=np.int32)

    peak_bins = max(num_sources, int(peak_bins))
    bin_edges = np.linspace(0.0, 360.0, peak_bins + 1)
    hist, _ = np.histogram(angles_deg % 360.0, bins=bin_edges, weights=weights)
    # Light circular smoothing keeps one strong source from splintering into adjacent bins.
    hist = (
        np.roll(hist, 2)
        + 2.0 * np.roll(hist, 1)
        + 3.0 * hist
    ...
    for candidate_idx in np.argsort(hist)[::-1]:
        if hist[candidate_idx] <= 0:
            break
        candidate = float(bin_centers[candidate_idx])
        if all(
            circular_angle_error_deg(candidate, selected) >= min_separation_deg
            for selected in selected_centers
        ):
            selected_centers.append(candidate)
            if len(selected_centers) == num_sources:
                break

    if len(selected_centers) < num_sources:
        for idx in np.argsort(weights)[::-1]:
            candidate = float(angles_deg[idx] % 360.0)
            if all(
                circular_angle_error_deg(candidate, selected) >= min_separation_deg
                for selected in selected_centers
            ):
    ...
    if len(selected_centers) < num_sources:
        for idx in np.argsort(weights)[::-1]:
            candidate = float(angles_deg[idx] % 360.0)
            if all(circular_angle_error_deg(candidate, selected) > 1e-6 for selected in selected_centers):
                selected_centers.append(candidate)
                if len(selected_centers) == num_sources:
                    break
    ...
    labels = assign_points_to_circular_centers(angles_deg, selected_centers)
    refined_centers = []
    for source_id in range(num_sources):
        mask = labels == source_id
        if np.any(mask):
            refined_centers.append(circular_mean_deg_360(angles_deg[mask], weights[mask]))
        else:
            refined_centers.append(selected_centers[source_id] % 360.0)

    order = np.argsort(np.asarray(refined_centers))
    remap = {int(old): int(new) for new, old in enumerate(order)}
    sorted_centers = [int(round(refined_centers[int(old)])) % 360 for old in order]
    sorted_labels = np.asarray([remap[int(label)] for label in labels], dtype=np.int32)
    return sorted_centers, sorted_labels


def cluster_doa_from_tensors(
    doa_est: torch.Tensor,
    vad_est: torch.Tensor,
    num_sources: int,
    vad_th: float,
    strategy: str = "separated_peaks",
    ...
    empty_plot_data = {
        ...
        "tracks": np.asarray(valid_tracks, dtype=np.int32),
        ...
        "all_tracks": np.asarray(all_tracks, dtype=np.int32),
    }
    ...
    valid_tracks_np = np.asarray(valid_tracks, dtype=np.int32)
    ...
    elif strategy == "separated_peaks":
        final_doas, labels = separated_peak_cluster_doas(
            angles_deg=valid_angles_np,
            weights=valid_weights_np,
            num_sources=num_sources,
            min_separation_deg=min_separation_deg,
            peak_bins=peak_bins,
        )
        if len(final_doas) < num_sources:
            return [], empty_plot_data
    else:
        raise ValueError(f"Unsupported DoA clustering strategy: {strategy}")
    ...
    plot_data = {
        ...
        "tracks": valid_tracks_np,
        "labels": labels.astype(np.int32),
        "centers": np.asarray(final_doas, dtype=np.float32),
        "cluster_strategy": np.asarray([strategy]),
        "all_times": np.asarray(all_times, dtype=np.float32),
        "all_angles": np.asarray(all_angles, dtype=np.float32),
        "all_scores": np.asarray(all_scores, dtype=np.float32),
        "all_tracks": np.asarray(all_tracks, dtype=np.int32),
...
def postprocess_doa_from_tensors(
    ...
    doas, _ = cluster_doa_from_tensors(
        doa_est=doa_est,
        vad_est=vad_est,
        num_sources=num_sources,
        vad_th=vad_th,
...
def save_doa_cluster_plot(
    ...
    scores = np.asarray(plot_data.get("scores", []), dtype=np.float32)
    ...
    elif all_frame_indices.size:
        max_frame = max(float(np.max(all_frame_indices)), 1.0)
        times_sec = frame_indices
    else:
        times_sec = frame_indices
    if all_frame_indices.size:
        all_max_frame = max(float(np.max(all_frame_indices)), 1.0)
        all_times_sec = all_frame_indices / all_max_frame * duration_sec
    else:
        all_times_sec = all_frame_indices

    fig, ax = plt.subplots(figsize=(10, 4.5))
    if all_angles.size:
        score_min = float(np.min(all_scores)) if all_scores.size else 0.0
        score_max = float(np.max(all_scores)) if all_scores.size else 1.0
        ...
        cbar = fig.colorbar(all_scatter, ax=ax, pad=0.01)
    ...
    if selected_doa is not None and selected_doa >= 0:
        ax.axhline(
            float(selected_doa),
            color="red",
            linestyle="-",
            linewidth=2.0,
            label=f"Selected idx {selected_idx}, DoA {selected_doa} deg",
        )

    ax.set_title(f"Clustered predicted DoA points - chunk {chunk_index}")
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("Estimated azimuth (degree, 0-360)")
    ax.set_xlim(0, max(duration_sec, 0.001))
    ax.set_ylim(0, 360)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=True,
        help="Save per-chunk SSL DoA clustering plots under <out_dir>/DoA.",
    )
    parser.add_argument(
        "--raw_input_gain",
        type=float,
...
def main() -> None:
    ...
    doa_plot_dir = args.out_dir / "DoA"
    if args.save_doa_plots:
        doa_plot_dir.mkdir(parents=True, exist_ok=True)
    ...
    doa_plot_warning_printed = False
    ...
    def save_doa_plot_if_enabled(
        *,
        ...
        nonlocal doa_plot_warning_printed
        if not args.save_doa_plots:
            return
        try:
            save_doa_cluster_plot(
                plot_path=plot_path,
                plot_data=plot_data,
            ...
            if not doa_plot_warning_printed:
                print(f"Warning: failed to save DoA cluster plots: {exc}")
                doa_plot_warning_printed = True
    ...
    try:
        ...
        with source_context as chunk_source:
            ...
            for chunk_index, wav_tc in tqdm(
                ...
                pred_doas, doa_plot_data = cluster_doa_from_tensors(
                    doa_est=ssl_out["doa_est"],
                    vad_est=ssl_out["vad_est"],
                ...
                if len(pred_doas) != args.num_sources:
                    save_doa_plot_if_enabled(
                        plot_path=doa_plot_dir / f"doa_chunk{chunk_index:06d}_no_selection_points{len(doa_plot_data['angles'])}.png",
                        plot_data=doa_plot_data,
                        duration_sec=duration_sec,
                        chunk_index=chunk_index,
                        selected_doa=None,
                        selected_idx=None,
                ...
                save_doa_plot_if_enabled(
                    plot_path=doa_plot_dir / (
                        ...
                        f"selected_idx{selected_idx}_doa{selected_doa}_"
                        f"preds_{'-'.join(str(doa) for doa in pred_doas)}.png"
                    ),
                    plot_data=doa_plot_data,
                    duration_sec=duration_sec,
                    chunk_index=chunk_index,
    ...
    summary = {
        ...
        "doa_plot_dir": str(doa_plot_dir) if args.save_doa_plots else "",
        ...
        "save_doa_plots": args.save_doa_plots,
