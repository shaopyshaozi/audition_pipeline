"""
Offline IPDNet/GT-DOA -> MVDR beamforming -> Whisper WER benchmark.

For each saved multichannel scene, this script evaluates the dominant speaker
target only (spk1) and can run two classical
beamforming variants:

1. MVDRBeamformer-GT: steer once to each ground-truth speaker DOA parsed from the
   target text/clean filenames.
2. MVDRBeamformer-IPDNet: run IPDNet once, match its three predicted DOAs to the
   spk1 GT speaker DOA, then steer to the matched predicted DOA.
...
def load_target_references(
    ...
    for clean_path in sorted(clean_dir.glob(f"clean_fileid_*_doa*_spk{target_speaker_id}.wav")):
...
def mvdr_beamform(
    ...
    # Reference steering vector to microphone 0
    relative_delays = relative_delays - relative_delays[0]
    ...
    y_ft = np.empty((x_fct.shape[0], x_fct.shape[2]), dtype=np.complex128)
    eye = np.eye(num_mics, dtype=np.complex128)

    for freq_idx, freq in enumerate(freqs):
        x_mt = x_fct[freq_idx].astype(np.complex128, copy=False)
        steering = np.exp(1j * 2.0 * np.pi * float(freq) * relative_delays).astype(np.complex128)
        ...
        try:
            inv_cov_steering = np.linalg.solve(cov_loaded, steering)
        except np.linalg.LinAlgError:
            inv_cov_steering = np.linalg.pinv(cov_loaded) @ steering

        denom = steering.conj().T @ inv_cov_steering
        if abs(denom) < 1e-12:
            weights = steering / float(num_mics)
        else:
            weights = inv_cov_steering / denom

        y_ft[freq_idx] = weights.conj().T @ x_mt
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run GT/IPDNet DOA -> MVDR -> Whisper WER benchmark.")
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Speaker id to evaluate. Default is 1 for the dominant speaker.",
    )
    parser.add_argument(
        "--doa_source",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1e-3,
        help="Diagonal loading factor for covariance inversion. Smaller is more aggressive; larger is more stable.",
    )
...
def main() -> None:
    ...
    enhanced_dir = args.out_dir / "pipeline_mvdr_enhanced"
    if args.save_enhanced:
    ...
    print(
        f"Beamformer: MVDR, n_fft={args.beam_n_fft}, hop={args.beam_hop}, "
        f"diag_load={args.mvdr_diag_load}"
    )
    ...
    print(
        f"Target spk{args.target_speaker_id} text references: "
        f"{sum(len(v) for v in references_by_fileid.values())}"
    )
    ...
    for fileid, target_paths in tqdm(grouped.items(), desc="MVDR-ASR", unit="scene"):
        refs = references_by_fileid.get(fileid, [])
        if not refs:
            skipped_no_gt_targets += 1
            print(
                f"fileid={fileid}: no text_fileid_*_doa*_spk{args.target_speaker_id}.txt "
                "reference, skipped."
            )
            continue
        if len(refs) > 1:
            print(
                f"fileid={fileid}: found {len(refs)} spk{args.target_speaker_id} "
                "references; evaluating the first sorted reference."
            )
            refs = refs[:1]
            ...
            for ref in refs:
                enhanced, beam_sec = elapsed_seconds(
                    ...
                    lambda ref=ref: mvdr_beamform(
                        ...
                        diagonal_loading=args.mvdr_diag_load,
                    ),
                )
        ...
        if args.doa_source in ("ipdnet", "both"):
            ...
            for ref in refs:
                ...
                enhanced, beam_sec = elapsed_seconds(
                    "cpu",
                    lambda steering_doa=steering_doa: mvdr_beamform(
                        wav_tc,
                        sr,
    ...
    details_csv = args.out_dir / f"pipeline_whisper_{args.whisper_model}_mvdr_wer_details_1asr.csv"
    ...
    summary_json = args.out_dir / f"pipeline_whisper_{args.whisper_model}_mvdr_wer_summary_1asr.json"
    summary_json.write_text(json.dumps(summary, indent=2), encoding="utf-8")

    print("\n===== MVDR ASR SUMMARY =====")
    print(f"Target speaker: spk{summary['target_speaker_id']}")
    print(f"Evaluated utterances: {summary['evaluated_utterances']}")
    print(f"Skipped scenes without target speaker text refs: {summary['skipped_no_gt_targets']}")
