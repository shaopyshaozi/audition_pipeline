...
def run_sepformer_mono_file(
    *,
    ...
    mono_16k, sr = load_mono_audio(mixture_path, target_sr=args.sample_rate)
    mono_8k = resample_1d(mono_16k, args.sample_rate, args.sepformer_sample_rate)
    tmp_dir.mkdir(parents=True, exist_ok=True)
    safe_tag = re.sub(r"[^A-Za-z0-9_.-]+", "_", case_tag)
    sepformer_input = tmp_dir / f"sepformer_input_{safe_tag}_8k.wav"
    sf.write(str(sepformer_input), mono_8k, args.sepformer_sample_rate)
...
class Libri3MixCase:
    ...
    utterance_ids: Tuple[str, str, str]


def split_to_librispeech_subset(split: str) -> str:
    aliases = {
        "test": "test-clean",
        "dev": "dev-clean",
...
def resolve_libri3mix_base(args: argparse.Namespace) -> Path:
    if args.libri3mix_root is None:
        raise ValueError(
            "--libri3mix_root is required when --eval_dataset libri3mix. "
            "Expected a generated Libri3Mix folder containing wav8k/min/test or wav16k/min/test."
        )

    rate_dir = f"wav{args.libri3mix_sample_rate // 1000}k"
    candidates = [
        args.libri3mix_root / rate_dir / args.libri3mix_mode / args.libri3mix_split,
        args.libri3mix_root / "Libri3Mix" / rate_dir / args.libri3mix_mode / args.libri3mix_split,
    ]
    ...
    raise FileNotFoundError(
        "Could not find the Libri3Mix split folder. Tried:\n"
        f"  {tried}\n"
        "The standard layout is Libri3Mix/wav8k/min/test/{mix_clean,s1,s2,s3}."
    )


def discover_libri3mix_cases(args: argparse.Namespace) -> List[Libri3MixCase]:
    split_base = resolve_libri3mix_base(args)
    mixture_dir = split_base / args.libri3mix_mixture
    source_dirs = [split_base / f"s{idx}" for idx in range(1, args.num_sources + 1)]
    if args.num_sources != 3:
        raise ValueError("Libri3Mix evaluation expects --num_sources 3.")
    if not mixture_dir.is_dir():
        raise FileNotFoundError(f"Libri3Mix mixture folder not found: {mixture_dir}")
    missing_source_dirs = [path for path in source_dirs if not path.is_dir()]
    if missing_source_dirs:
        raise FileNotFoundError(f"Libri3Mix source folders not found: {missing_source_dirs}")

    mixture_paths = sorted(mixture_dir.glob("*.wav"))
    if args.max_items > 0:
        mixture_paths = mixture_paths[: args.max_items]

    cases: List[Libri3MixCase] = []
    for case_index, mixture_path in enumerate(mixture_paths):
        source_paths = tuple(source_dir / mixture_path.name for source_dir in source_dirs)
        missing_sources = [path for path in source_paths if not path.is_file()]
        if missing_sources:
            raise FileNotFoundError(f"Missing source files for {mixture_path.name}: {missing_sources}")

        case_id = mixture_path.stem
        utterance_ids = tuple(case_id.split("_"))
        if len(utterance_ids) != args.num_sources:
            raise ValueError(
                f"Expected Libri3Mix mixture id to contain 3 utterance ids separated by '_', got: {case_id}"
            )
        cases.append(
            Libri3MixCase(
                ...
                utterance_ids=utterance_ids,  # type: ignore[arg-type]
            )
        )
    return cases


def infer_librispeech_root(libri3mix_root: Optional[Path]) -> Optional[Path]:
    if libri3mix_root is None:
        return None
    candidates = [
        libri3mix_root / "LibriSpeech",
        libri3mix_root.parent / "LibriSpeech",
        libri3mix_root.parent.parent / "LibriSpeech",
    ]
    for candidate in candidates:
        if candidate.is_dir():
            return candidate
...
def load_librispeech_transcripts(librispeech_root: Optional[Path], subset: str) -> Dict[str, str]:
    ...
    for trans_path in subset_dir.glob("*/*/*.trans.txt"):
...
def load_transcript_overrides(path: Optional[Path], num_sources: int) -> Dict[Tuple[str, int], str]:
    if path is None:
        return {}
    if not path.is_file():
        raise FileNotFoundError(f"Transcript CSV not found: {path}")

    id_columns = ("mixture_ID", "mixture_id", "case_id", "ID", "id")
    text_columns = {
        idx: (
            f"source_{idx}_text",
            f"source_{idx}_transcript",
            f"s{idx}_text",
            f"s{idx}_transcript",
            f"speaker_{idx}_text",
            f"speaker_{idx}_transcript",
        )
    ...
    with path.open("r", newline="", encoding="utf-8") as f:
        ...
        for row in reader:
            case_id = next((row[col] for col in id_columns if col in row and row[col]), "")
            case_id = Path(case_id).stem
            if not case_id:
                continue
            for speaker_id, columns in text_columns.items():
                text = next((row[col] for col in columns if col in row and row[col]), "")
                if text:
                    overrides[(case_id, speaker_id)] = text.strip()
    return overrides


def build_libri3mix_references(
    case: Libri3MixCase,
    transcript_by_utt: Dict[str, str],
    transcript_overrides: Dict[Tuple[str, int], str],
    allow_missing: bool,
) -> List[TargetReference]:
    references: List[TargetReference] = []
    missing: List[str] = []
    for speaker_id, (source_path, utt_id) in enumerate(zip(case.source_paths, case.utterance_ids), start=1):
        transcript = transcript_overrides.get((case.case_id, speaker_id)) or transcript_by_utt.get(utt_id)
        if transcript is None:
            missing.append(utt_id)
            transcript = ""
        references.append(
            TargetReference(
                fileid=case.case_index,
                speaker_id=speaker_id,
                gt_doa=0,
                text_path=None,
    ...
    if missing and not allow_missing:
        raise FileNotFoundError(
            f"Missing transcripts for Libri3Mix case {case.case_id}: {missing}. "
            "Pass --librispeech_root pointing at LibriSpeech, pass --libri3mix_transcript_csv, "
            "or use --allow_missing_transcripts to run ASR/metrics without WER references."
        )
    return references
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--eval_dataset",
        type=str,
        choices=("local_4mic", "libri3mix"),
        default="local_4mic",
        help="Use the repo 4-mic Eval set or generated Libri3Mix test cases.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Generated Libri3Mix root, e.g. /data/Libri3Mix or its parent. Required with --eval_dataset libri3mix.",
    )
    parser.add_argument("--libri3mix_split", type=str, default="test", help="Libri3Mix split folder, usually test or dev.")
    parser.add_argument("--libri3mix_mode", type=str, default="min", choices=("min", "max"))
    parser.add_argument("--libri3mix_sample_rate", type=int, default=8000, choices=(8000, 16000))
    parser.add_argument(
        "--libri3mix_mixture",
        type=str,
        default="mix_clean",
        choices=("mix_clean", "mix_both"),
        help="Use clean speech mixtures or speech+noise mixtures.",
    )
    parser.add_argument(
        "--librispeech_root",
        type=Path,
        default=None,
        help="LibriSpeech root used to read test-clean/dev-clean *.trans.txt references for Libri3Mix WER.",
    )
    parser.add_argument(
        "--libri3mix_transcript_csv",
        type=Path,
        default=None,
        help="Optional CSV with per-source transcript columns such as source_1_text, s2_transcript, etc.",
    )
    parser.add_argument(
        "--allow_missing_transcripts",
        action="store_true",
        help="Run Libri3Mix ASR and audio metrics even when transcript references are missing; WER fields stay empty.",
    )
...
def save_sources(
    *,
    ...
    for source_idx in range(sources_16k_st.shape[0]):
        if args.eval_dataset == "libri3mix":
            name_16k = f"sepformer_libri3mix_case{fileid}_{stem}_source{source_idx + 1}_16k.wav"
        else:
        ...
        if args.save_8k_sources:
            if args.eval_dataset == "libri3mix":
                name_8k = f"sepformer_libri3mix_case{fileid}_{stem}_source{source_idx + 1}_8k.wav"
            else:
...
def append_result_for_target(
    *,
    ...
    ref_text = ref.transcript if ref.transcript is not None else ref.text_path.read_text(encoding="utf-8").strip()
    if ref_text:
        sample_wer, dist, ref_word_count = wer(ref_text, hyp_text)
    else:
        sample_wer, dist, ref_word_count = None, None, None
    ...
    return AsrTiming(
        ...
        gt_text_file=ref.text_path.name if ref.text_path is not None else "",
        wer=sample_wer,
        edit_distance=dist,
...
def validate_common_runtime_args(args: argparse.Namespace) -> None:
...
def run_libri3mix_eval(args: argparse.Namespace) -> None:
    validate_common_runtime_args(args)
    cases = discover_libri3mix_cases(args)
    if not cases:
        raise FileNotFoundError("No Libri3Mix mixture wav files were found.")
    ...
    librispeech_root = args.librispeech_root or infer_librispeech_root(args.libri3mix_root)
    librispeech_subset = split_to_librispeech_subset(args.libri3mix_split)
    transcript_by_utt = load_librispeech_transcripts(librispeech_root, librispeech_subset)
    transcript_overrides = load_transcript_overrides(args.libri3mix_transcript_csv, args.num_sources)
    ...
    print(f"Eval dataset: Libri3Mix {args.libri3mix_split}/{args.libri3mix_mode}/{args.libri3mix_mixture}")
    print(f"Libri3Mix root: {args.libri3mix_root}")
    print(f"LibriSpeech root: {librispeech_root if librispeech_root else 'not found'}")
    print(f"Transcript references loaded: {len(transcript_by_utt) + len(transcript_overrides)}")
    print(f"Selected Libri3Mix cases: {len(cases)}")
    print(f"Target speaker: {'all' if args.target_speaker_id <= 0 else f's{args.target_speaker_id}'}")
    ...
    skipped_missing_transcripts = 0

    for case in tqdm(cases, desc="Libri3Mix SepFormer-ASR", unit="mix"):
        refs = build_libri3mix_references(
            case,
            transcript_by_utt=transcript_by_utt,
            transcript_overrides=transcript_overrides,
            allow_missing=args.allow_missing_transcripts,
        )
        if args.target_speaker_id > 0:
            refs = [ref for ref in refs if ref.speaker_id == args.target_speaker_id]
        skipped_missing_transcripts += sum(1 for ref in refs if not ref.transcript)

        sources_16k_st, sources_8k_st, noisy_ref_for_metrics, sepformer_sec, sr = run_sepformer_mono_file(
            separator=separator,
            mixture_path=case.mixture_path,
            case_tag=case.case_id,
            args=args,
            tmp_dir=tmp_dir,
        )
        if sources_16k_st.shape[0] < len(refs):
            ...
            print(
                f"{case.case_id}: expected at least {len(refs)} separated sources, "
        ...
        if args.save_enhanced or args.save_8k_sources:
            saved_16k_names = save_sources(
                ...
                fileid=case.case_index,
                mic_file=case.mixture_path.name,
        ...
        for ref in refs:
            if ref.speaker_id not in matches:
                skipped_missing_match += 1
                print(f"{case.case_id}, s{ref.speaker_id}: no SepFormer source match, skipped.")
                continue
    ...
    details_csv = args.out_dir / f"libri3mix_whisper_{args.whisper_model}_sepformer_wer_details_{target_label}.csv"
    ...
    summary = {
        "eval_dataset": "libri3mix",
        "libri3mix_root": str(args.libri3mix_root),
        "librispeech_root": str(librispeech_root) if librispeech_root else "",
        "libri3mix_split": args.libri3mix_split,
        "libri3mix_mode": args.libri3mix_mode,
        "libri3mix_sample_rate": args.libri3mix_sample_rate,
        "libri3mix_mixture": args.libri3mix_mixture,
        "whisper_model": args.whisper_model,
        "whisper_device": args.whisper_device,
        "device": args.device,
        "enhancement": "speechbrain_sepformer_libri3mix_single_mic",
        "sepformer_source": args.sepformer_source,
        ...
        "source_matching": "oracle_si_sdr_libri3mix_sources",
        "selected_cases": len(cases),
        "evaluated_utterances": len(scene_results),
        "missing_transcript_references": skipped_missing_transcripts,
        "skipped_missing_match": skipped_missing_match,
        "save_enhanced": args.save_enhanced,
        "enhanced_dir": str(enhanced_dir) if (args.save_enhanced or args.save_8k_sources) else "",
        "overall": all_summary,
        "by_method": by_method,
    }

    summary_json = args.out_dir / f"libri3mix_whisper_{args.whisper_model}_sepformer_wer_summary_{target_label}.json"
    summary_json.write_text(json.dumps(summary, indent=2), encoding="utf-8")

    print("\n===== LIBRI3MIX SEPFORMER ASR SUMMARY =====")
    print(f"Evaluated utterances: {summary['evaluated_utterances']}")
    print(f"Missing transcript references: {summary['missing_transcript_references']}")
