...
This script does not implement MUSIC, SourceTracker, or GHDSS in Python. It is
for the official HARK baseline:

    4-channel wav
        -> official HARK network:
           LocalizeMUSIC -> SourceTracker/SourceTrackerPF -> GHDSS
        -> separated wavs
        -> Whisper small
        -> WER / SDRi / SI-SDRi
...
1. run_official:
   call an official HARK .n network once per test scene.
2. eval_official:
   evaluate the separated wavs produced by official HARK with the same Whisper
   and metric code used by the other baselines.

The HARK network must accept an input wav path and an output prefix/directory.
If your .n file uses a different CLI shape, set --hark_command_template.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hark_network", type=Path, default=None, help="Official HARK .n network file.")
    ...
    parser.add_argument(
        ...
        default="oracle_wer",
        help=(
            "Choose the enhanced stream by transcribing every HARK output and selecting "
            "the one with the lowest WER against the reference text. This is an oracle "
            "upper-bound selection used for this HARK baseline."
        ),
    )
...
def scene_output_prefix(args: argparse.Namespace, fileid: int, mic_path: Path) -> Path:
    return scene_output_dir(args, fileid) / f"hark_fileid_{fileid}_{mic_path.stem}"
...
def patch_hark_network_for_scene(args: argparse.Namespace, mic_path: Path, fileid: int) -> Path:
...
def render_hark_command(args: argparse.Namespace, mic_path: Path, fileid: int) -> List[str]:
    if args.hark_network is None:
        raise ValueError("--hark_network is required for --mode run_official or --mode both.")
    ...
    runtime_network = patch_hark_network_for_scene(args, mic_path, fileid)
    values = {
...
def run_official_hark(args: argparse.Namespace) -> List[HarkRunRecord]:
    ...
    for fileid, target_paths in tqdm(grouped.items(), desc="Official-HARK", unit="scene"):
        ...
        command = render_hark_command(args, mic_path, fileid)
...
def evaluate_official_outputs(args: argparse.Namespace) -> List[EvalRecord]:
    ...
    asr_cache: Dict[str, Tuple[str, float, np.ndarray]] = {}
    skipped = {
        "no_target_refs": 0,
        "no_hark_wavs": 0,
        "selection_failed": 0,
    ...
    for fileid, target_paths in tqdm(grouped.items(), desc="Eval-official-HARK", unit="scene"):
        ...
        for ref in target_refs:
            ...
            try:
                candidates = []
                for idx, wav_path in enumerate(separated_wavs):
                    cache_key = str(wav_path.resolve())
                    if cache_key not in asr_cache:
                        candidate_audio, _ = load_mono_audio(wav_path, target_sr=sr)

                        def run_candidate_asr(candidate_audio=candidate_audio):
                        ...
                        asr_out, candidate_whisper_sec = elapsed_seconds(
                            args.whisper_device,
                            run_candidate_asr,
                        )
                        asr_cache[cache_key] = (
                            asr_out.get("text", "").strip(),
                            candidate_whisper_sec,
                            candidate_audio,
                        )

                    candidate_hyp, _, _ = asr_cache[cache_key]
                    candidate_wer, candidate_dist, candidate_ref_words = wer(ref_text, candidate_hyp)
                    candidates.append(
                        (
                            candidate_wer,
                            candidate_dist,
                ...
                if not candidates:
                    raise ValueError("No candidate HARK wavs found for WER-oracle selection.")
                (
                    sample_wer,
                    dist,
                    selected_index,
                ...
                selected_key = str(selected_wav.resolve())
                enhanced = asr_cache[selected_key][2]
                whisper_sec = sum(asr_cache[str(path.resolve())][1] for path in separated_wavs)
                pred_doa = None
                doa_error = None
